	...
	.controller('HomeCtrl', ['$scope', 'dataSvc',
		function ($scope, dataSvc) {
			...
			$scope.updateSyncObject = function () {
				WL.JSONStore.get('syncStore').findAll().then(function (resSyncStore) {
					var objId = resSyncStore[resSyncStore.length - 1]._id; // id of the object to be replaced

					WL.JSONStore.get('syncStore').syncReplace(objId,
						{ // attach object data
							objectSync: $scope.objectSync
						},
						{ // true to mark the dirty flag
							markDirty: true
						}).then(function (response) {
							WL.JSONStore.get('syncStore').findById(objId)
								.then(function (res) {
									$scope.$apply(function () {
										$scope.syncData = JSON.stringify(res);
									});
								})
						}).catch(function (error) {
							$scope.$apply(function () {
								$scope.syncData = JSON.stringify(error);
							});
						});
				});
			}
