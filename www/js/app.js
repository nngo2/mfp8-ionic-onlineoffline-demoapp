angular.module('app', ['ionic', 'ngCordova', 'app.controllers', 'app.services', 'app.qExtension'])
	...
	.run(function ($q, $ionicPlatform, $rootScope, $cordovaDialogs, mfpPromise) {
		$ionicPlatform.ready(function () {
			...
			// Test jsonstore enhanced method (update a timestamp for every new syncReplace method)
			mfpPromise.then(function () {
				// initialize the store with timestamp column
				var collections = {
					syncStore: {
						objectSync: {
							searchFields: {
				...
				WL.JSONStore.init(collections, { password: 'password', username: 'username', localKeyGen: true })
					.then(function () {
						return WL.JSONStore.get('syncStore').count();
					})
					.then(function (countResult) { // populate test data
						if (countResult === 0) { // The JSONStore collection is empty, populate it.                
							var data = [{ objectSync: { clientVersion: Date.now(), field1: 'aaa', field2: 'bbb' } }];
							return WL.JSONStore.get('syncStore').add(data);
						}
					});

				// enhanced Replace
				WL.JSONStore.get('syncStore').enhance('syncReplace', function (id, obj, options) {
					var deferred = $q.defer();

					if (obj.objectSync) {
						obj.objectSync.clientVersion = Date.now();

						WL.JSONStore.get('syncStore').replace({
							_id: id,
							json: {
								objectSync: obj.objectSync
							}
						}, options).then(deferred.resolve, deferred.reject);
					}
